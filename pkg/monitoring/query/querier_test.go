...
var _ = Describe("Querier", func() {
	...
	BeforeEach(func() {
		...
		querier = query.NewQuerierWithConfig(tmpDir, 5*time.Second)
	})
	...
	Describe("QueryOrderbook", func() {
		It("should return orderbook for asset", func() {
			...
			mux.HandleFunc("/api/orderbook", func(w http.ResponseWriter, r *http.Request) {
				...
				Expect(exchange).To(Equal("binance"))
				Expect(pair).To(Equal("BTC-USDT"))
	...
	Describe("QueryPredictionOrderbook", func() {
		It("should return prediction orderbook for market and outcome", func() {
			mux := http.NewServeMux()
			mux.HandleFunc("/api/orderbook/prediction", func(w http.ResponseWriter, r *http.Request) {
				marketID := r.URL.Query().Get("market_id")
				outcomeID := r.URL.Query().Get("outcome_id")
				Expect(marketID).To(Equal("market-123"))
				Expect(outcomeID).To(Equal("outcome-yes"))

				orderbook := connector.OrderBook{
					Bids: []connector.PriceLevel{
						{Price: numerical.NewFromFloat(0.65), Quantity: numerical.NewFromFloat(100)},
					},
					Asks: []connector.PriceLevel{
						{Price: numerical.NewFromFloat(0.67), Quantity: numerical.NewFromFloat(80)},
					},
				}
				json.NewEncoder(w).Encode(orderbook)
			})
			...
			result, err := querier.QueryPredictionOrderbook(instanceID, "market-123", "outcome-yes")
			Expect(err).NotTo(HaveOccurred())
			Expect(result.Bids).To(HaveLen(1))
			Expect(result.Asks).To(HaveLen(1))
		})
	...
	Describe("QueryRecentTrades", func() {
		It("should return recent trades with limit", func() {
			...
			mux.HandleFunc("/api/trades", func(w http.ResponseWriter, r *http.Request) {
				...
				trades := []connector.Trade{
					{ID: "trade-1", Pair: portfolio.NewPair(portfolio.NewAsset("BTC"), portfolio.NewAsset("USDT"))},
					{ID: "trade-2", Pair: portfolio.NewPair(portfolio.NewAsset("ETH"), portfolio.NewAsset("USDT"))},
				}
