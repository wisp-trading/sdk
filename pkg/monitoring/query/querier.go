...
func (q *querier) QueryPredictionOrderbook(instanceID string, marketID prediction.MarketID, outcomeID prediction.OutcomeID) (*connector.OrderBook, error) {
	var result connector.OrderBook
	path := fmt.Sprintf("/api/orderbook/prediction?market_id=%s&outcome_id=%s", marketID.String(), outcomeID.String())
	if err := q.doRequest(instanceID, path, &result); err != nil {
		return nil, err
	}
	return &result, nil
